    ...
    return [
      m('.fish-table',
        ...
        m(Table, {
          ...
          noRowsText: 'No se encontró nada'
        })
      )
    ]
  }
    ...
    return [
      m('.col-sm-8',
        m(FilterGroup, {
          ...
          filters: {
            'Todo': () => { vnode.state.filteredRecords = vnode.state.records },
            'Propiedades': () => filterRecords((record) => record.owner === publicKey),
            'Administración': () => filterRecords((record) => record.custodian === publicKey),
            'Reportando': () => filterRecords(
