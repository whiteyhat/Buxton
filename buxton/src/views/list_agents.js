    ...
    return [
      m('.agent-list',
        ...
        m(Table, {
          ...
          noRowsText: 'No hay administradores'
        })
     )
    ]
  }
    ...
    return [
      m('.col-sm-8',
        m(FilterGroup, {
          ...
          filters: {
            'Todos': () => { vnode.state.filteredAgents = vnode.state.agents },
            'Propietarios': () => filterAgents(agent => agent.owns.length > 0),
            'Administradores': () => filterAgents(agent => agent.custodian.length > 0),
